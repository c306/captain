...
Ext.ux.TreeCombo = Ext.extend(
				Ext.form.TriggerField,
				{
					...
					//This ensures that after trigger click a value will be selected in a tree if it was set.
					...
					getTree : function() {
						if (!this.treePanel) {
							...
							this.treePanel = new Ext.tree.TreePanel(
									{
										...
										rootVisible : (typeof this.rootVisible != 'undefined') ? this.rootVisible
							...
							if (this.resizable) {
								this.resizer = new Ext.Resizable(this.treePanel
										.getEl(), {
									pinned : true,
									handles : 'se'
								});
					...
					onExpandOrCollapseNode : function() {
						...
						var heightPadding = treeEl.getHeight()
								- treeEl.dom.clientHeight;
						var ulEl = treeEl.child('ul'); // Get the underlying
														// tree element
					...
					collapse : function() {
						this.getTree().hide();
						if (this.resizer)
							this.resizer.resizeTo(this.treeWidth,
									this.treeHeight);
					},
					...
					onRender : function(ct, position) {
						Ext.ux.TreeCombo.superclass.onRender.call(this, ct,
								position);
						if (this.hiddenName) {
